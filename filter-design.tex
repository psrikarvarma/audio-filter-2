...
\author{Penmetsa Srikar Varma\\EE23BTECH11215}
\date{}
...
range for our bandpass filter as $7.0$ kHz - $8.2$ kHz.  Hence, the un-normalized discrete time filter
...
\begin{align}
    ...
    \omega_{p2} &= 2\pi\frac{F_{p2}}{F_s}  =0.34 \pi
\end{align}
...
\begin{align}
    F_{s1} &= 7.0-0.3 = 6.7 \text{KHz}\\
    F_{s2} &= 8.2+0.3 = 8.5  \text{KHz}
\end{align}
\begin{align}
    \omega_{s1} &= 2\pi\frac{F_{s1}}{F_s} = 0.279 \pi\\
     \omega_{s2} &= 2\pi\frac{F_{s2}}{F_s} = 0.354 \pi\\
\end{align}
\end{enumerate}
...
$\Omega_{p1} = 0.489$, $\Omega_{p2} = 0.591$ and $\Omega_{s1} = 0.468$, $\Omega_{s2} = 0.621$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.5382$ and $B = \Omega_{p2} - \Omega_{p1} = 0.1015$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.473\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.53
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.473.
\end{align}
...
we get $N\geq 3$ and $0.278 \leq \epsilon \leq 0.61$\\
\newpage
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/1.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/2.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/3.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
Substituting respective values in \eqref{eq:Gain_eq_LP} and executing following python code, we get $G_{LP}=0.4166$
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/g_lp.py
\end{lstlisting}
\begin{align}
...
\end{align}
The below code plots the Design vs Specification plot.
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/4.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
Substituting \eqref{eq:res1} in \eqref{eq:res2} and executing following code, we obtain Gain of required bass pass filter:
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/g_bp.py
\end{lstlisting}
\begin{align}
    G_{BP} &= 1.034 
\end{align}
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{4.42\times 10^{-5}s^4}{s^8 + 0.132s^7 + 1.179s^6 + 0.1160s^5 + 0.516s^4 + 0.033s^3 + 0.099s^2 + 0.00322s + 0.00708} \label{eq:magnitude_bandpass_sdom}
\end{align}
The below code gives actual above expression
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/10.py
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/5.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
Substituting $s=\frac{1-z^{-1}}{1+z^{-1}}$ in \eqref{eq:magnitude_bandpass_sdom} and calculating expression below python code we get,
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/11.py
\end{lstlisting}
\begin{align}
    H_{d,BP}(z) &= \frac{G\brak{1 - 3.99z^{-2} + 5.99z^{-4} - 3.99z^{-6} + z^{-8}}}{3.085 - 13.202z^{-1} + 32.841z^{-2} - 52.588z^{-3} + 60.811z^{-4} - 49.978z^{-5} + 29.651z^{-6} - 11.324z^{-7} + 2.516z^{-8}}
\end{align}

where $G=4.42\times 10^{-5}$    
...
\begin{align}
    \omega_{l} &= \frac{B}{2}\\
                &= 0.025\pi
\end{align}
The below code plots the low pass filter frequency and impulse response plot.
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/6.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
\end{enumerate}
The below code plots the FIR low pass filter plot.
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/7.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
The below code plots the FIR band pass filter plot.
\begin{lstlisting}
https://github.com/psrikarvarma/filter-design/blob/main/codes/8.py
\end{lstlisting}

\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/8.png}
